...
exports.articleFor = articleFor;
exports.pluralOf = pluralOf;
exports.classOf = classOf;
exports.examplesFor = examplesFor;
exports.pluralPhrase = pluralPhrase;
...
function isVowelStart(word) {
    ...
    if (w in AN_EXCEPTIONS)
        return AN_EXCEPTIONS[w];
...
function classOf(word, packKey) {
    const w = String(word ?? '').trim().toLowerCase();
    if (!w)
        return undefined;
    if (w in OVERRIDES)
        return OVERRIDES[w] ?? undefined;
 ...
 * 只有字母类用得上。
 */
function examplesFor(word, packKey, topic) {
...
function pluralPhrase(word, packKey) {
    if (classOf(word, packKey) !== 'countNoun')
        return undefined;
