    ...
    for (let i = 0; i < 200; i++) {
      ...
      /*
        ...
        **第一步完全没意义**。
        ...
        而「把已知条件列出来」恰恰是这整件事的核心:
        条件得由他自己从画面里数出来,不是被告知。
        所以题面只说清是什么情况,数量全部交给图。
      */
      ok(
        !/[0-9]|[一二三四五六七八九十]个/.test(q.story),
        `题目里不该出现数量 —— 那样第一步就只是复读(${q.story})`,
      )
      /*
        反过来:条件必须**在图里数得出来**。
        题面不说数量了,如果图也没有,这道题就无解了。
      */
      ok(!!q.visual && q.visual.groups.length >= 1, `题面不说数量时,图必须给得出来(${q.story})`)
