...
export const BUILTIN_PACKS: BuiltinPackMeta[] = [
  ...
  // ---------------- 古诗 ----------------
  {
    key: 'poems-primary',
    /*
      rev 2(v69):同题不同作者的诗要在标题上分得开。
      「出塞」「赤壁」「渡汉江」「凉州词」都有两首,而卡片的 front 就是标题,
      错题本和线下抽查里会并排出现两个「出塞」—— 家长不知道该考哪一首。
      升 rev 是为了让**已经装在设备上**的卡组重新同步一次标题
      (改的是建卡逻辑,不是 JSON,不升 rev 的话老卡组永远不会刷新)。
    */
    rev: 2,
    name: '小学·唐诗启蒙',
    subject: '语文',
