import type { RecordFieldValue, RecordModule } from '../types'

/**
 * 配置驱动的通用记录引擎(从网页版原样搬过来)。
 *
 * 视力、牙齿、就医、体检、疫苗、考级、获奖、情绪、阅读 —— 这些本质上
 * 都是「一条带日期的记录 + 几个字段」,没必要各写一个页面。
 * 这里用一份配置描述每个模块有哪些字段,一个通用页面按配置渲染。
 * 以后要加新记录类型,只加一段配置即可。
 */
...
export const GROUP_LABEL: Record<RecordModuleGroup, string> = {
  health: '健康',
  learning: '阅读学习',
  talent: '兴趣特长',
  wellbeing: '情绪心理',
...
export const RECORD_MODULES: RecordModuleDef[] = [
  ...
  },
  {
    ...
    fields: [
      { key: 'symptom', label: '症状', type: 'text', required: true, placeholder: '如「发烧38.5度、咳嗽」' },
  ...
  },
  {
    ...
    addLabel: '新增体检记录',
    hasNote: true,
    disclaimer: '体检结论请以体检机构报告为准;身高体重请在「生长曲线」里登记,才能算进百分位。',
    fields: [
  ...
  },
  {
    ...
    trendFields: [{ key: 'intensity', label: '强度', color: '#7c8cff' }],
    summarize: (f) =>
...
export function modulesByGroup(group: RecordModuleGroup): RecordModuleDef[] {
  return RECORD_MODULES.filter((m) => m.group === group)
}

/** 品格维度预设:事例记录用来给具体行为打标签(记录行为,不给孩子贴标签) */
