 ...
 * 录音和放音的互斥闸。
 *
 * 为什么要有:麦克风和喇叭在同一台手机上**是一件事**。
 * 一边录自己的声音、一边点「逐条朗读」,结果是两个声音同时往外放,
 ...
 * 录音期间任何播放请求直接不响应。**
 *
 * 为什么单独一个文件:recorder 需要「开录之前把音停掉」,
 * audio 需要「放之前问一句是不是在录」—— 两边互相 import 会成环。
 * 这个文件谁都不依赖,双方各依赖它一次,环就断了。
 */

let recording = false

/** 各个播放器注册进来的「停止」动作(在线音源一个、本机文件一个) */
const stoppers: Array<() => void> = []

/** 兜底解锁:开录后最多锁这么久。录音本身上限 15 秒,留足余量 */
const MAX_LOCK_MS = 25000
 ...
 * 开始录音。
 *
 * 先停声再上锁,顺序不能反:反过来的话 stopAllAudio 里如果有异步收尾,
 * 尾音仍可能漏出半秒被录进去。
 */
export function beginRecording(): void {
  ...
  /*
    为什么要兜底:孩子按下录音之后直接退页面 / 切后台,
    onStop 有可能永远不来。那时候锁如果不自动开,
    整个 app 从此一点声音都没有 —— 这种坏法比声音打架严重得多。
  */
