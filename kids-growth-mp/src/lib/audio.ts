...
function ordered(list: AudioSource[], prefKey: string, bucket: LenBucket): AudioSource[] {
  ...
  /*
   * 英语整句**一律先走百度合成引擎**,即使家长选了「美音·真人」。
   *
   * 这不是无视用户的选择,而是这两样东西根本不是一类:
   * 有道 dictvoice 是**词典** —— 单词有真人录音(质量最好),
   * 但整句它要么没有音频、要么把句子当词条拼出来,听着就是
   * 一个词一个词往外蹦。用户反馈的「有读不出来的、会自动单词读」
   * 就是这么来的:他选了有道真人,于是整句也被顶到有道去了。
   *
   * 所以分工是固定的:
   *   单词 → 家长选的音色说话(有道真人最自然)
   *   整句 → 百度合成引擎(任意句子都能连贯读出来)
   * 家长的音色选择在**单词**这一档完全生效,那才是它该管的地方。
   */
