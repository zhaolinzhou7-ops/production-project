...
const STEPS: Record<Platform, { title: string; steps: string[]; note?: string }> = {
  ...
  ios: {
    title: '苹果手机:下载「增强版」语音(注意:列表里名字不会变)',
    steps: [
      '打开「设置」→「辅助功能」→「朗读内容」→「声音」',
      '选「中文(普通话)」→ 找 婷婷 / 美佳 这些,点右边的 ⬇️ 下载「增强版」;英语同理下载 Samantha 等的增强版',
      '下载完**把本页完全关掉再重新打开**(iPhone 上要刷新页面才会用上新音质)',
      '在下面「设备自带音色」里选中它,点一下试听',
    ],
    note:
      '苹果这里有两个坑,不是你操作错了:① 下载「增强版」后,音色名字**还是婷婷/Samantha**,不会新增一条,所以看起来"找不到"——但音质已经换了;② Siri 的声音苹果**不开放给网页使用**,所以网页里永远看不到 Siri 音色。',
  },
  wechat: {
    title: '你现在用的是微信内置浏览器',
    steps: [
